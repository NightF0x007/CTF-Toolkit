...
run_nmap() {
  ...
  # Extract Web Ports
  local web_ports=$(grep -oP '\d{1,5}/open/tcp//http' $nmap_out | awk -F '/' '{print $1}' | grep -vE '21|22|23|25|53|135|139|445|161|110|143|3306|3389')
...
run_dir_enum() {
  ...
  gobuster dir -u "http://$TARGET" -w "$DIR_WORDLIST" -q -t 50 -o "$dir_enum_out"
  if [ $? -ne 0 ]; then
